...
#define	MBX_ERR(mbx, fmt, arg...)	\
	pr_err("qep_drv: %s: "fmt"\n", mbx->mbx_pdev->name, ##arg)
#define	MBX_WARN(mbx, fmt, arg...)	\
	pr_warn("qep_drv: %s: "fmt"\n", mbx->mbx_pdev->name, ##arg)
#define	MBX_INFO(mbx, fmt, arg...)	\
	pr_info("qep_drv: %s: "fmt"\n", mbx->mbx_pdev->name, ##arg)
#define	MBX_DBG(mbx, fmt, arg...)	\
	pr_debug("qep_drv: %s: "fmt"\n", mbx->mbx_pdev->name, ##arg)
...
int  mailbox_connect(struct platform_device *pdev)
{
	...
	size_t data_len = 0,  reqlen = 0, resplen = 0;
	void *kaddr = NULL;
	struct mailbox *mbx = platform_get_drvdata(pdev);
	int ret;

	data_len = sizeof(struct xcl_mailbox_conn);
	resplen = sizeof(struct xcl_mailbox_conn_resp);
	reqlen = sizeof(struct xcl_mailbox_req) + data_len;

	resp = (struct xcl_mailbox_conn_resp *) vzalloc(resplen);
	if (!resp) 
		goto done;
	...
	ret = mailbox_request(pdev, mb_req, reqlen, resp, &resplen,
		NULL, NULL, 0);
	if(!ret)
		goto done;

	(void) mailbox_set(pdev, CHAN_STATE, resp->conn_flags);
	(void) mailbox_set(pdev, CHAN_SWITCH, resp->chan_switch);
	(void) mailbox_set(pdev, COMM_ID, (u64)(uintptr_t)resp->comm_id);

	MBX_INFO(mbx, "ch_state 0x%llx, connected \n", resp->conn_flags);
...
#else	
	mbx->mbx_regs = (struct mailbox_reg *)pdev->resource;
#endif
